drop table if exists `mimic-iv-ches.cohorts.mimic4ds_inhospmort`;
create table `mimic-iv-ches.cohorts.mimic4ds_inhospmort` as(
    with icu_last as (
    ...
    select
        ...
        -- LABEL
        case 
            when a.deathtime <= datetime_add(i.intime, interval 7 day)
            then 1 
            else 0 end as label
    from
        icu_last i
...
select * from `mimic-iv-ches.cohorts.mimic4ds_inhospmort`