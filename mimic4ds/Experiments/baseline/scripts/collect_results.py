...
2. bootstrap CI of model comparisons 
    - compare baseline model (trained on 2008 - 2010) with 
      oracle models for each year group (i.e., models trained
      on each subsequent year group)
"""
...
parser.add_argument(
    "--save_path",
    type = str,
    default = "/hpf/projects/lsung/projects/public/mimic4ds_public/mimic4ds/Experiments/baseline/results",
)
...
def get_CI_evaluation(args,df,evaluation_method):
    ...
    train_year = df['train_group'].unique()[0]
    eval_year = df['eval_group'].unique()[0]
    ...
    for phase in phases:
        for metric in args['metrics']:
            ...
            i_df = df.query("phase==@phase and metric==@metric")
    ...
    for phase in phases:
        for dem_cat in dem_cats:
            for metric in args['metrics']:
                ...
                i_df = df.query(
                    f"phase==@phase and {demographic_category}==@dem_cat and metric==@metric"
                )
...
def get_CI_comparison(args,df,evaluation_method):
    ...
    # gather metadata
    phases = df['phase'].unique()
    train_year_1 = df['train_group_1'].unique()[0]
    train_year_2 = df['train_group_2'].unique()[0]
    eval_year = df['eval_group'].unique()[0]
    task = df['analysis_id'].unique()[0]
